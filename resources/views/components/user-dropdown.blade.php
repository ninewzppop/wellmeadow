        ...
        <div class="border-b border-slate-100 px-4 py-3">
            ...
            @if ($user->staff)
                <p class="mt-1 text-xs text-slate-500">
                    ...
                    @if ($user->staff->positions->first()?->pos)
                        · {{ $user->staff->positions->first()->pos->Pos_Name }}
                    @endif
                    @if ($user->staff->Alloc_Wd_No)
                        · {{ $user->staff->Alloc_Wd_No }}
                    @endif
                </p>
            @endif
        </div>

        <a href="{{ route('password.edit') }}" role="menuitem"
            class="mt-2 flex w-full items-center gap-2 px-4 py-2 text-left text-sm font-medium text-slate-700 transition hover:bg-slate-100">
