...
@section('content')
    @php
        $editing = $item->exists;
        $route = $editing ? route('stock.update', $item) : route('stock.store');
    @endphp

    <div class="mb-6">
        <h1 class="text-2xl font-bold text-[#112D6E]">
            {{ $editing ? __('Edit item').' — '.$item->Item_No : __('New item') }}
        </h1>
    </div>
    ...
    <form method="POST" action="{{ $route }}" class="space-y-8">
        ...
        <section class="rounded-lg bg-white p-6 shadow">
            <h2 class="mb-4 text-sm font-semibold uppercase tracking-wide text-slate-500">{{ __('Item details') }}</h2>
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                ...
                <div>
                    <label for="ItemType" class="block text-sm font-medium text-slate-700">{{ __('Category *') }}</label>
                    <select id="ItemType" name="ItemType" required
                            class="mt-1 w-full rounded border border-slate-300 px-3 py-2 text-sm focus:border-sky-500 focus:outline-none">
                        @foreach (\App\Models\CentralStock::itemTypes() as $value => $label)
                            <option value="{{ $value }}" @selected(old('ItemType', $item->ItemType) === $value)>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
        ...
        <div class="flex items-center gap-3">
            <button type="submit"
                ...
                {{ $editing ? __('Save changes') : __('Create item') }}
            </button>
            <a href="{{ route('stock.index') }}" class="text-sm text-slate-600 hover:text-slate-900">{{ __('Cancel') }}</a>
        </div>
    </form>

    @if ($editing)
        <section id="stock-actions" class="mt-8 grid grid-cols-1 gap-6 lg:grid-cols-2">
            <form method="POST" action="{{ route('stock.restock', $item) }}" class="rounded-lg bg-white p-6 shadow">
                @csrf
            ...
            <form method="POST" action="{{ route('stock.adjust', $item) }}" class="rounded-lg bg-white p-6 shadow">
                @csrf
