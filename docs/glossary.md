...
| surgical / non-surgical | The two values of `CentralStock.ItemType`; the Stock page's category filter and search vocabulary. |
...
| Confirm dispense & payment | Single combined pharmacy action (`POST /medications/{order}/confirm`) that, in one transaction, checks `Pharmaceutical.QtyInStock ≥ UnitsPerDay×daysInclusive` per item (lockForUpdate), deducts stock and writes `StockMovement`, creates one `Medications` history row per order item (`MD{n}`) and marks the order `dispensed` (`PaidAt = DispensedAt = now()`), removing it from the queue. Blocked with `Cannot dispense` when out-of-stock. |
| Ward requisition | A store-to-ward supply request (`Wardrequisitions` header `WR{n}` + `Itemrequest`/`Drugrequest` lines). Lifecycle `Pending → Approved (stock deducted + StockMovement) → Completed (Received_By + DateRecv, in history)`. Total cost is live `Σ QtyReq × CostPerUnit`. |
| Requisition queue | Store worklist (`/requisitions`) of `Wardrequisitions` with `status IN (Pending,Approved)` ordered by `DateOrd` — pending needs approval, approved awaits receipt. |
| Requisition history/report | `Completed` requisitions per ward and date range (`DateRecv` filter) plus a low-stock list (`QtyInStock ≤ ReorderLvl`) for supplies & drugs. |