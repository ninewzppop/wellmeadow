...
@section('content')
    ...
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-slate-900">{{ $editing ? 'Edit staff member' : 'New staff member' }}</h1>
    </div>
    ...
    <form method="POST"
        ...
        <section class="rounded-lg bg-white p-6 shadow">
            <h2 class="mb-4 text-sm font-semibold uppercase tracking-wide text-slate-500">Personal details</h2>
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                <div>
                    <label for="Stf_No" class="block text-sm font-medium text-slate-700">Staff No *</label>
                ...
                <div>
                    <label for="NIN" class="block text-sm font-medium text-slate-700">National Insurance No</label>
                ...
                <div>
                    <label for="TelNo" class="block text-sm font-medium text-slate-700">Telephone</label>
            ...
            <div id="positions" class="space-y-3">
                @foreach ($staffPositions as $sp)
                    <div class="grid grid-cols-1 gap-3 rounded border border-slate-200 p-3 md:grid-cols-5">
                        ...
                        <div>
                            <label class="block text-xs font-medium text-slate-500">Position *</label>
                        ...
                        <div>
                            <label class="block text-xs font-medium text-slate-500">Salary</label>
                        ...
                        <div>
                            <label class="block text-xs font-medium text-slate-500">Hours / wk</label>
                            <input type="number" step="0.01" name="positions[{{ $loop->index }}][HrsPerWk]" value="{{ $sp->HrsPerWk }}"
                                   class="mt-1 w-full rounded border border-slate-300 px-2 py-1.5 text-sm">
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-slate-500">Contract type</label>
                            <input type="text" name="positions[{{ $loop->index }}][ContractType]" value="{{ $sp->ContractType }}"
                                   class="mt-1 w-full rounded border border-slate-300 px-2 py-1.5 text-sm">
                        </div>
                        <div class="flex items-end gap-2">
                            <div class="flex-1">
                                <label class="block text-xs font-medium text-slate-500">Payment type</label>
        ...
        <section class="rounded-lg bg-white p-6 shadow">
            <div class="mb-4 flex items-center justify-between">
                <h2 class="text-sm font-semibold uppercase tracking-wide text-slate-500">Qualifications</h2>
            ...
            <div id="qualifications" class="space-y-3">
                @foreach ($qualifications as $q)
                    <div class="grid grid-cols-1 gap-3 rounded border border-slate-200 p-3 md:grid-cols-4">
                        ...
                        <div>
                            <label class="block text-xs font-medium text-slate-500">Institution</label>
        ...
        <section class="rounded-lg bg-white p-6 shadow">
            <div class="mb-4 flex items-center justify-between">
                <h2 class="text-sm font-semibold uppercase tracking-wide text-slate-500">Work experience</h2>
            ...
            <div id="experiences" class="space-y-3">
                @foreach ($workExperiences as $e)
                    <div class="grid grid-cols-1 gap-3 rounded border border-slate-200 p-3 md:grid-cols-5">
                        ...
                        <div>
                            <label class="block text-xs font-medium text-slate-500">Organization</label>
                        ...
                        <div>
                            <label class="block text-xs font-medium text-slate-500">Start date</label>
                            <input type="date" name="work_experiences[{{ $loop->index }}][StartDate]" value="{{ optional($e->StartDate)->format('Y-m-d') }}"
                                   class="mt-1 w-full rounded border border-slate-300 px-2 py-1.5 text-sm">
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-slate-500">Finish date</label>
        ...
        <div class="flex items-center gap-3">
            ...
            <a href="{{ route('staff.index') }}" class="text-sm text-slate-600 hover:text-slate-900">Cancel</a>
        </div>
    </form>
